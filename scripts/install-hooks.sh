...
install_pre_commit() {
    ...
    local source_hook="hooks/pre-commit"
    local target_hook="$HOOKS_DIR/pre-commit"

    if [ ! -f "$source_hook" ]; then
        echo -e "${RED}❌ pre-commit hook not found at $source_hook${NC}"
        echo "  Please ensure the hook file exists in the hooks directory"
        return 1
    fi
    ...
    # Verify hook content
    if grep -q "Enhanced pre-commit hook for Kelime İşlem" "$target_hook"; then
...
install_pre_push() {
    ...
    local source_hook="hooks/pre-push"
    local target_hook="$HOOKS_DIR/pre-push"

    if [ ! -f "$source_hook" ]; then
        echo -e "${RED}❌ pre-push hook not found at $source_hook${NC}"
        echo "  Please ensure the hook file exists in the hooks directory"
        return 1
    fi
    ...
    # Verify hook content
    if grep -q "Optimized pre-push hook for Kelime İşlem" "$target_hook"; then
